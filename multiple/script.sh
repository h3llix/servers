...
# <(printf "\n[SAN]\nsubjectAltName=DNS.1:localhost,DNS.3:localhost,DNS.4:0.0.0.0,DNS.5:host.docker.internal,DNS.6:127.0.0.1")) \
...
#
# -extfile <(printf "subjectAltName=DNS.1:localhost,DNS.3:localhost,DNS.4:0.0.0.0,DNS.5:host.docker.internal,DNS.6:127.0.0.1")
...
openssl x509 -req -in $domainCrs -CA $rootCACrt -CAkey $rootCAKey -CAcreateserial -days 1825 -sha256 -out $domainCrt \
  -extfile <(printf "subjectAltName=DNS.1:localhost")
